...
The DOM behaves like a programming interface, it abstract the necessary entities you will need to display content on HTML. 
Under the hood, the DOM is composed by various API's that allow us to interact with the document structure while the core only job
is to define the structure (or attributes)

JS: You are only allowed one default export per module. If there is a default export you can import it directly without curly braces:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=carlosreynaet%40gmail.com).
